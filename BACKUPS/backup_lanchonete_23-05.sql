...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COMMENT='Registra as principais alterações neste BD.';
...
INSERT INTO `auditoria` (`codAuditoria`, `acao`, `tabela`, `dataHora`, `usuario`) VALUES
	...
	(3, 'Produto excluido= 9', 'produto', '2023-05-23 10:29:14', 'root@localhost'),
	(4, 'Produto excluido= 8', 'produto', '2023-05-23 10:31:06', 'root@localhost');
...
INSERT INTO `produto` (`codProduto`, `nome`, `precoCusto`, `precoVenda`, `margemLucro`, `dataValidade`, `quantidadeEstoque`, `quantidadeMinima`, `CATEGORIA_codCategoria`, `MARCA_codMarca`) VALUES
	...
	(4, 'Serenata de Amor', 1.00, 2.00, 100.00, '2023-12-16', 30.00, 10.00, 6, 2);
...
-- Copiando estrutura para trigger lanchonete3si_2023.tri_AtulaizaEstoque
DROP TRIGGER IF EXISTS `tri_AtulaizaEstoque`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tri_AtulaizaEstoque` AFTER UPDATE ON `itemvenda` FOR EACH ROW BEGIN
...
if(new.quantidade > OLD.quantidade)
	...
	ELSE if (NEW.quantidade < OLD.quantidade)	
				then UPDATE produto SET quantidadeEstoque = quantidadeEstoque + (OLD.quantidade - newquantidade)
				WHERE codProduto = NEW.PRODUTO_codProduto;
	END if;
END if;	
END//
